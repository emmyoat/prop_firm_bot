...
class TelegramNotifier:
    ...
    def get_updates(self):
        ...
        try:
            ...
            logger.error(f"Error polling Telegram updates: {self._redact(str(e))}")
        return []

    def _redact(self, text: str) -> str:
        """Helper to hide token from logs"""
    ...
    def send_message(self, message: str):
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(f"Failed to send Telegram message: {self._redact(response.text)}")
            else:
